...
public class DPA_InitializerConfigurationFactory {
    ...
    private static RoutingHopsConfiguration createRoutingHopsConfiguration(
            Configuration configuration
    ) {
        int doRoutingHops = configuration.getInt("initialization.type.dpa.routingHops", -1);
        if (doRoutingHops == 0) {
            return null;
        }
        
        int requestHops = configuration.getInt(
                "initialization.type.dpa.routingHops.requestHops", -1
        );
        
        int responseHops = configuration.getInt(
                "initialization.type.dpa.routingHops.responseHops", -1
        );
        
        // if user explicitly sets to do routing hops
        if (doRoutingHops > 0) {
            if (requestHops == -1) {
                requestHops = RoutingHopsConfiguration.DEFAULT_REQUEST_HOPS;
            }
            
            if (responseHops == -1) {
                responseHops = RoutingHopsConfiguration.DEFAULT_RESPONSE_HOPS;
            }
        }
        
        return new RoutingHopsConfiguration(requestHops, responseHops);
    }
    ...
    public static DPA_InitializerConfiguration getDPA_InitializerConfiguration(
        ...
        RoutingHopsConfiguration routingHopsConfig = 
                createRoutingHopsConfiguration(configuration);
