...
void DpaLedG(uint16_t addr, uint8_t cmd) {
	...
	app_vars.myDpaRequest.HWPID = HWPID_DoNotCheck;
...
void MyDebugMessages(void) {
		...
		if (app_vars.debugRequest) {
			...
			sprintf(app_vars.debugString + sizeof(requestToSend), "%x    %x    %x    %x    ", app_vars.myDpaRequest.NADR \
													...
													, app_vars.myDpaRequest.HWPID);

			app_vars.ptrRequest = (char*)&app_vars.myDpaRequest.DpaMessage.Request.PData;
			...
			sprintf(app_vars.debugString + sizeof(confirmationToSend), "%x    %x    %x    %x    ", app_vars.myDpaAnswer.NADR \
													     ...
													     , app_vars.myDpaAnswer.HWPID);

			// message data
			sprintf(app_vars.debugString + sizeof(confirmationToSend) + 4*5, "%x    %x    %x    %x    %x    ", app_vars.myDpaAnswer.ResponseCode \
															 , app_vars.myDpaAnswer.DpaValue \
															 , app_vars.myDpaAnswer.DpaMessage.IFaceConfirmation.Hops \
															 , app_vars.myDpaAnswer.DpaMessage.IFaceConfirmation.TimeSlotLength \
															 , app_vars.myDpaAnswer.DpaMessage.IFaceConfirmation.HopsResponse);
			...
			// message data
			sprintf(app_vars.debugString + sizeof(responseToSend) + 4*5, "%x    %x    ", app_vars.myDpaAnswer.ResponseCode \
												   , app_vars.myDpaAnswer.DpaValue );

			app_vars.ptrAnswer = (char*)&app_vars.myDpaAnswer.DpaMessage.Response.PData;
			//first 5 elemets from extension
			for(i=0; i<len; i++) {
