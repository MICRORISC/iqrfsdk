...
void printErrorAndExit(
    const char* userMessage, int error, int retValue
) {
...
// try to wait for communication ready state in specified timeout (in ms)
rpi_spi_iqrf_SPIStatus tryToWaitForDataReadyState(uint32_t timeout) {
    rpi_spi_iqrf_SPIStatus spiStatus = { 0, RPISPIIQRF_SPI_DISABLED };
    int operResult = -1;
    uint32_t elapsedTime = 0;
    struct timespec sleepValue = { 0, INTERVAL_MS };
...
int main() {
    ...
    for ( sendCounter = 0; sendCounter < 10; sendCounter++ ) {
        ...
        do {
            ...
            if (operResult) {
                printErrorAndExit(
                        "Error during data reading", 0, operResult
                        );
            }
            ...
            // reading confirmation and response, 2 cycles of reading sequence
            receiveCounter++;
        }
